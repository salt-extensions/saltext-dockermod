...
def tests(session):
    ...
    try:
        ...
        session.run(
            ...
            "--omit=src/saltext/dockermod/*",
            "--include=tests/*",
        )
        try:
            session.run("coverage", "report", "--show-missing", "--include=src/saltext/dockermod/*")
            ...
            #    "--include=src/saltext/dockermod/*,tests/*"
            # )
        finally:
